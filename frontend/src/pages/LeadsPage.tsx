    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <form onSubmit={create} className="space-y-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <input className="w-full rounded-lg border border-slate-300 px-3 py-2" placeholder="Nome" value={name} onChange={(e) => setName(e.target.value)} />
          <input className="w-full rounded-lg border border-slate-300 px-3 py-2" placeholder="Telefone" value={phone} onChange={(e) => setPhone(e.target.value)} />
          <button className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">Salvar</button>
        </form>
        <form onSubmit={importCsv} className="space-y-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="font-medium">Importar CSV</h2>
          <textarea className="h-28 w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-xs" value={csv} onChange={(e) => setCsv(e.target.value)} />
          <button className="rounded-lg bg-slate-100 px-4 py-2 text-sm font-medium text-slate-800">Importar</button>
